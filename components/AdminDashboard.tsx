...
const AdminDashboard: React.FC = () => {
  ...
  const handleFormSubmit = async (data: Omit<Article, 'id'|'date'|'imageUrl'>) => {
    try {
      if (articleToEdit) {
        console.log('[AdminDashboard] Updating article:', articleToEdit.id);
        await updateArticle(articleToEdit.id, data);
        alert('✅ Article updated successfully!');
      } else {
        console.log('[AdminDashboard] Adding new article');
        await addArticle(data);
        alert('✅ Article added successfully! It should appear in the list below.');
      }
      setArticleToEdit(null); // Clear edit mode on success
    } catch (error: any) {
      console.error('[AdminDashboard] Failed to submit article:', error);
      ...
      if (error.code === 'permission-denied') {
        alert(
          ...
          '2. Select your project: fearp-47bae\n' +
